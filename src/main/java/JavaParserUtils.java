...
public class JavaParserUtils {
    ...
    static Map<String, String> InsideMethodMap=new HashMap<>();
...
//        }
    ...
    public static String getParents(final MethodCallExpr callExpr) {
        final StringBuilder path = new StringBuilder();

        callExpr.walk(Node.TreeTraversal.PARENTS, new Consumer<Node>() {
            @Override
            public void accept(Node node) {
                if (node instanceof ClassOrInterfaceDeclaration) {
                    path.insert(0, ((ClassOrInterfaceDeclaration) node).getNameAsString());
...
//            getParents(n);
            if(InsideMethodMap.get(n.getName())==null) {
                InsideMethodMap.put(n.getNameAsString(), getParents(n));
            }else{
                InsideMethodMap.put(n.getNameAsString(), InsideMethodMap.get(n.getNameAsString())+"_"+(getParents(n)));
            }
            super.visit(n, arg);
        }
        @Override
